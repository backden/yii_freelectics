...
(function($, window, document, undefined) {
  ...
  var Comment = {
    ...
    buildItemList_: function(comment_info) {
      ...
      // Like button
      var like_container = $('<span></span>');
      like_container.addClass('post-like');

      var text = self.options.buttons.like;
      if (comment_info.liked == true) {
        text = self.options.buttons.unlike;
      }
      var like = $('<a>' + comment_info.total_like + '&nbsp;' + text + '</a>');
      like.attr('href', '#');
      like.attr('status', comment_info.liked);
      like.attr('title', self.options.buttons.like);

      like_container.append(like);
      $(like).click(function() {
        // Ex: posted-Comment_54aee1bf4395
        var that = this;
        $.get(
                self.options.url_like + "&comment_id=" + comment_id,
                function(data) {
                  if (self.options.onLike != undefined) {
                    self.options.onLike(data);
                    if (data.status == 'OK') {
                      var comment_id = $(that).parent().parent().parent().parent().attr("id");
                      comment_id = comment_id.split("posted-")[1];
                      if ($(that).attr('status') == "true") {
                        $(that).text(data.total + " " + self.options.buttons.like);
                        $(that).attr('status', false);
                      } else {
                        $(that).text(data.total + " " + self.options.buttons.unlike);
                        $(that).attr('status', true);
                      }
                    } else {
                      alert(data.error);
                    }
      ...
      comments[comment_id] = {likes: comment_info.total_like};

      post_foot.append(like_container);
      ...
      if (self.user_info_.is_edit_allowed && (comment_info.created_by == self.user_info_.user_id))
      {
        ...
        var edit = $('<a>' + self.options.buttons.edit + '</a>');
        edit.attr('href', '#');
        edit.attr('title', self.options.buttons.edit);
      ...
      if (self.user_info_.is_edit_allowed && (comment_info.created_by == self.user_info_.user_id))
      {
        ...
        var delete_ = $('<a>' + self.options.buttons.delete + '</a>');
        delete_.attr('href', '#');
        delete_.attr('title', self.options.buttons.delete);
      ...
      if (self.user_info_.is_add_allowed)
      {
        ...
        var reply = $('<a>' + self.options.buttons.reply + '</a>');
        reply.attr('href', '#');
        reply.attr('title', self.options.buttons.reply);
